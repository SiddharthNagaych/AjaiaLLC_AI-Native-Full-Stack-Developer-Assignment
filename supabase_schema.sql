...
-- Security Definer helper functions (prevents RLS infinite recursion between documents and document_shares)
...
create or replace function public.is_document_shared_with(doc_id uuid, user_id uuid)
returns boolean
language sql
security definer
set search_path = public
as $$
  select exists (
    ...
    where document_id = doc_id and shared_with_user_id = user_id
  );
$$;

-- Documents Policies
create policy "Owners and shared users can view documents." on public.documents
  for select using (auth.uid() = owner_id or is_document_shared_with(id, auth.uid()));

create policy "Owners can insert their own documents." on public.documents
  for insert with check (auth.uid() = owner_id);

create policy "Owners and shared users can update documents." on public.documents
  for update using (auth.uid() = owner_id or is_document_shared_with(id, auth.uid()));
...
create policy "Owners can manage shares for their documents." on public.document_shares
  for all using (is_document_owner(document_id, auth.uid()));
